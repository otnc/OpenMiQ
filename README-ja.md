...
`chain` は、引用するメッセージがさらに別のメッセージへのリプライだった場合、その2つを1枚の画像に上下連結して表示します(makeitaquoteの `MiQChain` を使用)。引用するメッセージがリプライでなければ `chain` は何もせず、通常どおり1枚のquote画像になります。`flip` と同様、縦画像レイアウトでは無視されます — 全面レイアウトには2枚のquoteを並べるための左右のアバター枠がないためです。他のトグルと異なり、生成済みのquote画像のボタンからは切り替えられないため、保存済みデフォルト(`/settings|/server-settings|/admin set chain:`)で設定するか、メンションの後ろに書いてください。
