...
生成された画像の下には、それぞれを切り替えられるボタン(🎨 カラー / 🅱️ 太字 / 🔄 反転 / ☀️ テーマ / レイアウト)、フォントを選択するセレクトメニュー、そしてカラーテーマ用のセレクトメニューが2つ(公式21種類用と、オリジナル18種類用)付きます — 両方を1つのセレクトメニューにまとめるとDiscordの25選択肢の上限を超えてしまうため分かれています。どちらも同じ設定を共有しているので、片方で選択するともう片方は自動的に「デフォルト」表示に戻ります。これらはすべて使うたびに画像を再生成しますが、それに加えて無効化されていなければ(下記スラッシュコマンド参照)🗑️ 削除ボタンも付きます。削除ボタンを押せるのは、その引用を生成した本人か、引用対象になった本人(引用元メッセージの投稿者、または `/fakequote` の `author:`)だけです。実際にメッセージを削除するのではなく、画像とコンポーネントを取り除く編集を行うだけなので、`SAVE_IMAGES_DIR` に保存されたコピーがあれば影響を受けません。
