...
@MiQ chain                      … リプライ元があれば、そのメッセージと連結する (横画像レイアウトのみ)
...
それぞれのオプションには逆の指定と1文字の短縮形があり(`font=`/`theme=`/`chain`を除く)、保存済みのデフォルト(下記スラッシュコマンド参照)がオンになっていても、そのメッセージだけ元に戻せます:
...
`chain` は、引用するメッセージがさらに別のメッセージへのリプライだった場合、その2つを1枚の画像に上下連結して表示します(makeitaquoteの `MiQChain` を使用)。引用するメッセージがリプライでなければ `chain` は何もせず、通常どおり1枚のquote画像になります。`chain` と `new` は併用できません — 全面レイアウトには2枚のquoteを並べるための左右のアバター枠がないためです — この場合 `chain` が優先され、`chain` が有効な間は強制的に横画像(`side`)レイアウトになります(同じ理由で、生成済みのquote画像のレイアウト切り替えボタンも `chain` が有効な間は無効化されます)。`chain` 自体は他のトグルと異なり生成済みのquote画像のボタンからは切り替えられないため、保存済みデフォルト(`/settings|/server-settings|/admin set chain:`)で設定するか、メンションの後ろに書いてください。
...
フォントは makeitaquote 自身の `FONT_ALIASES` のエイリアスで指定します: `sans`, `mplus`, `dela`, `dot`, `pop`, `rampart`, `reggae`, `rocknroll`, `serif`, `yuji`, `yusei`, `inconsolata`, `exo2`, `bruno`, `poltawski`, `vina`, `script`, `castoro`。makeitaquoteの正式なフォント名(例: `font="Dela Gothic One"`)も引き続き使用できます。何も指定しない場合のBot自体のデフォルトは `mplus` (M PLUS Rounded 1c) です。`theme=` は21種類の名前付き背景カラー(`sunset`, `forest`, `midnight_blurple` など)から選べます。[本家Make it a Quoteのテーマ一覧](https://wiki.neody.land/wiki/Make_it_a_Quote/Themes/ja)と同じ構成で、正式なキー、アンダースコアを抜いた表記(`midnightblurple`)、短縮コードがあるテーマはその短縮コード(`mb`)のいずれでも指定できます。さらに、この一覧とは別にmakeitaquoteが用意しているオリジナルテーマも18種類あります(`tokyo_night`, `emerald_depths`, `ruby_noir` など)。指定方法は同じ `theme=` ですが、エイリアスは公式の短いもの(2〜3文字)と被らないよう4文字以上になっています(`tokyo`, `emerald`, `ruby` など)。どちらのテーマも、それぞれコントラストのために文字色が固定されているため、指定すると `light`/`dark` は効かなくなります(`flip` と縦画像の関係と同様です) — `theme=default` (またはそのエイリアスの `theme=def`, `theme=b`, `theme=w`) で保存済みの指定を未設定に戻せます。3種類(フォント、公式テーマ、オリジナルテーマ)すべての一覧はローカライズされた `/help` を実行して確認してください。カラーテーマのセレクトメニューは、`pnpm run deploy:images` (または `deploy`) でアプリケーション絵文字として作成済みなら、それぞれ小さなグラデーション絵文字でプレビュー表示されます — 未作成の間はテキストのみの選択肢になります。
...
生成された画像の下には、それぞれを切り替えられるボタン(🎨 カラー / 🅱️ 太字 / 🔄 反転 / ☀️ テーマ / レイアウト)、フォントを選択するセレクトメニュー、そしてカラーテーマ用のセレクトメニューが2つ(公式21種類用と、オリジナル18種類用)付きます — 両方を1つのセレクトメニューにまとめるとDiscordの25選択肢の上限を超えてしまうため分かれています。どちらも同じ設定を共有しているので、片方で選択するともう片方は自動的に「デフォルト」表示に戻ります。これらはすべて使うたびに画像を再生成しますが、それに加えて引用文にDiscordの実際の書式(太字・斜体・下線・取り消し線)を適用するMarkdownボタンもあります — これがオンの間は太字ボタンが無効化されます(すべての文字に一律で太字を適用してしまうと、本来Markdownの太字が示すはずの区別が消えてしまうためです)。さらに無効化されていなければ(下記スラッシュコマンド参照)🗑️ 削除ボタンも付きます。削除ボタンを押せるのは、その引用を生成した本人か、引用対象になった本人(引用元メッセージの投稿者、または `/fakequote` の `author:`)だけです。実際にメッセージを削除するのではなく、画像とコンポーネントを取り除く編集を行うだけなので、`SAVE_IMAGES_DIR` に保存されたコピーがあれば影響を受けません。
...
- [`.github/assets/`](./.github/assets/) 内のアイコン・ロゴには、flopdesign 様のフリーフォント **あかずきんポップ** を使用しています: https://flopdesign.booth.pm/items/1748058
- 引用符アクセント部分には **M PLUS Rounded 1c**(SIL Open Font License 1.1)を使用しています — makeitaquote自身がデフォルトフォントとしてダウンロードするのと同じ書体です: https://fonts.google.com/specimen/M+PLUS+Rounded+1c
